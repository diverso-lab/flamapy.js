...
export async function executeFlamapyOperation(pyodide, operation, fileContent, args = [], kwarg = {}, type = TYPE_SOLVING.BASIC) {
    ...
    const result = await pyodide.runPythonAsync(`
            ...
            if ${JSON.stringify(type)} != 'basic':
                dm = DiscoverMetamodels()
                feature_model = dm.use_transformation_t2m("uvlfile.uvl",'fm') 
                model = dm.use_transformation_m2m(feature_model,${JSON.stringify(type)})    
                if ${JSON.stringify(type)} == 'pysat' or ${JSON.stringify(type)} == 'pysat_diagnosis':      
                    operation = dm.get_operation(model,'PySAT'+${JSON.stringify(operation)})
                else:
                    operation = dm.get_operation(model,'BDD'+${JSON.stringify(operation)})
                if kwarg and kwarg.get('configuration_path'):
                    configuration = dm.use_transformation_t2m(kwarg['configuration_path'],'configuration')
                    if kwarg.get('full_configuration'):
                        configuration.is_full=kwarg['full_configuration']
